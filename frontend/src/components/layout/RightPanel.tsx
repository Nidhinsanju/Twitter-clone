    ...
    <div className="flex flex-col gap-4 py-1.5">
      ...
      <div className="rounded-2xl border border-border">
        <div className="flex items-center gap-3 p-4">
          <TwitterLogo className="h-7 w-7 text-accent" />
          <div>
            <p className="text-[17px] font-extrabold leading-tight">Subscribe to Premium</p>
          </div>
        </div>
        <p className="px-4 pb-3 text-[13px] text-text-secondary">
          Subscribe to unlock new features and, if eligible, receive a share of ad
          revenue.
        </p>
        <div className="px-4 pb-4">
          <button className="rounded-full bg-accent px-4 py-1.5 text-[15px] font-bold text-white transition-colors hover:bg-accent-hover">
            Subscribe
          </button>
        </div>
      </div>
        ...
        {whoToFollow.map((user) => (
          <div key={user.id} className="flex items-center gap-3 px-4 py-3 hover:bg-hover">
            ...
            <div className="min-w-0 flex-1">
              <Link href={`/profile/${user.handle}`} className="block truncate text-[15px] font-bold hover:underline">
                {user.name}
              </Link>
